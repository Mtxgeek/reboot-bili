...
- ✅ 通过CDP（Chrome DevTools Protocol）管理浏览器窗口
- ✅ 先启动about:blank等待CDP就绪，再打开所有URL
- ✅ 定时重启机制，解决浏览器内存泄漏问题
- ✅ 系统资源监控（CPU、内存）
- ✅ 超过阈值自动重启
- ✅ 支持从配置文件读取URL列表和参数
...
- ✅ Google Chrome（自动检测）
- ✅ 支持自定义浏览器路径
- ✅ 使用默认用户数据目录，保留登录状态和Cookie

### CDP特性
- ✅ 通过CDP打开/关闭浏览器标签页
- ✅ 支持CDP刷新页面
- ✅ CDP不可用时自动回退到subprocess方式
- ✅ 启动前清理浏览器进程，确保CDP端口正常绑定
...
1. **浏览器启动**：
   - 从配置文件读取URL列表
   - 先启动浏览器打开about:blank，等待CDP就绪
   - 通过CDP接口打开所有配置的URL
   - CDP不可用时回退到subprocess方式

2. **CDP管理**：
   - 使用Chrome DevTools Protocol管理浏览器
   - 通过`/json/new`打开新标签页
   - 通过`/json/close`关闭标签页
   - 通过`/json/reload`刷新页面
...
- 确保使用默认用户数据目录
- 不要修改`--user-data-dir`参数
- 启动前会清理浏览器进程，请确保手动登录过所需网站
...
### v1.1.0 (2026-07-06)
- 修复CDP连接问题：先启动about:blank等待CDP就绪
- 添加默认用户数据目录支持，保留登录状态和Cookie
- 删除窗口排布和窗口置顶相关逻辑
- 添加CDP诊断功能，便于排查连接问题
- CDP不可用时自动回退到subprocess方式
